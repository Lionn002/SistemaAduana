// src/layouts/AduanaLayout.jsx
import React, { useEffect, useState } from 'react';
import { NavLink, Outlet, useNavigate } from 'react-router-dom';
import {
  Home, FileText, ClipboardList, Settings, LogOut, Clock, Car, QrCode
} from 'lucide-react';
...
export default function AduanaLayout() {
  ...
  useEffect(() => {
    if (!user || user.role !== 'ADUANA') navigate('/');
  }, [user, navigate]);
    ...
    <div className="min-h-screen flex bg-gray-50 dark:bg-gray-900">
      <aside className="w-64 bg-[#0b1f3a] dark:bg-gray-800 text-white flex flex-col justify-between shadow-lg">
        <div>
          ...
          <nav className="px-4 space-y-2">
            ...
            {commonDocs.map((doc, idx) => (
              <NavLink key={idx} to={`/funcionario-aduana/${doc.to}`} className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2 rounded ${isActive ? 'bg-[#0a192f]' : 'hover:bg-[#0a192f]'}`}>
            ...
            <NavLink to="/funcionario-aduana/escaneo-qr" className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-2 rounded ${isActive ? 'bg-[#0a192f]' : 'hover:bg-[#0a192f]'}`}>
              <QrCode className="w-5 h-5 text-white" /> Escaneo de Cédula (QR)
            </NavLink>

            <NavLink to="/funcionario-aduana/informes" className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-2 rounded ${isActive ? 'bg-[#0a192f]' : 'hover:bg-[#0a192f]'}`}>
        ...
        <div className="px-4 pb-6 space-y-2">
          <NavLink to="/funcionario-aduana/ajustes" className={({ isActive }) =>
            `flex items-center gap-3 px-4 py-2 rounded ${isActive ? 'bg-[#0a192f]' : 'hover:bg-[#0a192f]'}`}>
