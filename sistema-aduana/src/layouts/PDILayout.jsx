...
import {
  Home, FileText, ClipboardList, Settings, LogOut, Clock, Car, QrCode, UserPlus
} from 'lucide-react';
...
export default function PDILayout() {
  ...
  useEffect(() => {
    if (!user || user.role !== 'PDI') navigate('/');
  }, [user, navigate]);
    ...
    <div className="min-h-screen flex bg-gray-50 dark:bg-gray-900">
      <aside className="w-64 bg-[#0b1f3a] dark:bg-gray-800 text-white flex flex-col justify-between shadow-lg">
        <div>
          ...
          <nav className="px-4 space-y-2">
            ...
            {commonDocs.map((doc, idx) => (
              <NavLink key={idx} to={`/funcionario-pdi/${doc.to}`} className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2 rounded ${isActive ? 'bg-[#0a192f]' : 'hover:bg-[#0a192f]'}`}>
            ...
            <NavLink to="/funcionario-pdi/escaneo-qr" className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-2 rounded ${isActive ? 'bg-[#0a192f]' : 'hover:bg-[#0a192f]'}`}>
              <QrCode className="w-5 h-5 text-white" /> Escaneo de Cédula (QR)
            </NavLink>

            <NavLink to="/funcionario-pdi/informes" className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-2 rounded ${isActive ? 'bg-[#0a192f]' : 'hover:bg-[#0a192f]'}`}>
        ...
        <div className="px-4 pb-6 space-y-2">
          <NavLink to="/funcionario-pdi/ajustes" className={({ isActive }) =>
            `flex items-center gap-3 px-4 py-2 rounded ${isActive ? 'bg-[#0a192f]' : 'hover:bg-[#0a192f]'}`}>
